...
- 子網域（本機）：`https://form.lvh.me:3002`（auth:3000 / t-pass:3001 之後）
...
2. **資料庫建表**：
   ```bash
   npx prisma db push      # 依 schema 直接建表（最快，無 migration history）
   # 或 npx prisma migrate dev --name init
   ```

3. **HTTPS 憑證**（與 auth / t-pass 共用 mkcert）：
   ```bash
   mkcert -install
   mkdir -p certs && mkcert -key-file certs/form.lvh.me-key.pem -cert-file certs/form.lvh.me.pem form.lvh.me
   ```

4. **啟動**（⚠️ 本專案**禁用 `npm run dev`**）：
   ```bash
   npm run build
   npm run start:https     # https://form.lvh.me:3002
   ```
...
- 驗章四鐵則在 `src/lib/tpass-auth.ts`（照抄 t-pass 參考實作）：`algorithms:['EdDSA']` / issuer / audience / exp。
